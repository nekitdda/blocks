...
impl Reverb {
    /// Table lookup for the modulation LFOs. The phase is a 0→1 sawtooth, so
    /// `phase * LFO_TABLE_LEN` indexes the sine directly.
    #[inline(always)]
    fn lfo(phase: f32) -> f32 {
        // Clamp defensively so a rate that rounds past 1.0 can never index out
        // of bounds on the audio thread.
...
/// One period of a sine, sampled at `LFO_TABLE_LEN` points.
///
/// `f32::sin` is not a `const fn`, so the table is built once on first use
/// (`LazyLock`) and shared by every `Reverb` instance: 4 KB for the process
/// instead of 4 KB per effect, and a table lookup instead of a `sinf()` call
/// per sample per channel (~96k/s at 48 kHz) in the innermost loop.
static LFO_TABLE: std::sync::LazyLock<[f32; LFO_TABLE_LEN]> = std::sync::LazyLock::new(|| {
...
impl Effect for Reverb {
    fn process(&mut self, left: &mut [f32], right: &mut [f32]) {
        ...
        for (l, r) in left.iter_mut().zip(right.iter_mut()) {
            ...
            let lfo1 = Self::lfo(self.lfo1_phase);
            let lfo2 = Self::lfo(self.lfo2_phase);
